...
class PRReviewAction:
    ...
    def _match_files(self, files: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for file in files:
            ...
            if self.exclude_patterns and any(glob.fnmatch.fnmatch(filename, pattern) for pattern in self.exclude_patterns):
                continue
            
            # Check if file should be included
            if not self.include_patterns or any(glob.fnmatch.fnmatch(filename, pattern) for pattern in self.include_patterns):
    ...
    def run(self) -> Dict[str, Any]:
        ...
        try:
            ...
            pr_info = self.agent.execute_tool("get_pull_request", {
                "repo": repo_name,
                "pr_number": pr_number
            })
            
            # Get PR files
            pr_files = self.agent.execute_tool("get_pull_request_files", {
                "repo": repo_name,
                "pr_number": pr_number
            })
            ...
            for file in filtered_files:
                file_diff = self.agent.execute_tool("get_pull_request_diff", {
                    "repo": repo_name,
                    "pr_number": pr_number,
            ...
            # Post comment to PR
            self.agent.execute_tool("add_pull_request_comment", {
                "repo": repo_name,
                "pr_number": pr_number,
            ...
            for line in lines:
                if "summary" in line.lower() or "changes" in line.lower():
                    ...
                    continue
                elif "issue" in line.lower() or "bug" in line.lower() or "problem" in line.lower():
                    current_section = "details"
                    continue
                elif "suggest" in line.lower() or "improvement" in line.lower() or "recommend" in line.lower():
